...
export class InventoryService {
  ...
  inbound(payload: { warehouseId: string; skuId: string; skuName: string; quantity: number }, user?: User, meta?: Record<string, unknown>) {
    ...
    auditService.record({ action: 'UPDATE', module: 'INVENTORY', targetId: inventory.id, targetName: inventory.skuId, detail: { type: 'INBOUND', quantity: payload.quantity, before, after: inventory.quantity, netIncrease: inventory.quantity - before, ...meta } }, user);
    return inventory;
  }
